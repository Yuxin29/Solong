...
static char	*ft_read_buffer(int fd, char *left_c)
{
	...
	bytes_read = 1;
	temp = NULL;
	while (!left_c || (!ft_gnl_strchr(left_c, '\n') && bytes_read > 0))
	{
		...
		if (bytes_read == 0)
			break ;
		buffer[bytes_read] = '\0';
		temp = ft_gnl_strjoin(left_c, buffer);
		if (!temp)
			return (free(buffer), free(left_c), NULL);
		free(left_c);
		left_c = temp;
...
char	*get_next_line(int fd)
{
	...
	if (remainder[i] == '\n')
		i++;
	line = ft_gnl_substr(remainder, 0, i);
	if (!line)
		return (free(remainder), remainder = NULL, NULL);
	temp = ft_gnl_substr(remainder, i, ft_gnl_strlen(remainder) - i);
	free (remainder);
	remainder = temp;
